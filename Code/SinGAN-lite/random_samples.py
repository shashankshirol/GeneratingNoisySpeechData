...
if __name__ == '__main__':
    ...
    if dir2save is None:
        ...
        try:
            ...
            pass
        if opt.mode == 'random_samples':
            real, phase, sr, extras = functions.read_wav_spec(opt.input_wav_train, opt, need_sr=True, send_extra=True)
            functions.adjust_scales2image(real, opt)
            Gs, Zs, reals, NoiseAmp = functions.load_trained_pyramid(opt)
            in_s = functions.generate_in2coarsest(reals,1,1,opt)
            SinGAN_generate(Gs, Zs, reals, NoiseAmp, opt, gen_start_scale=opt.gen_start_scale, phase=phase, sr=sr, extras=extras)
            ...
            SinGAN_generate(Gs, Zs, reals, NoiseAmp, opt, in_s, scale_v=opt.scale_v, scale_h=opt.scale_h, phase=phase, sr=sr, extras=extras)
