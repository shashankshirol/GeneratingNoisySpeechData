...
def train(opt,Gs,Zs,reals,NoiseAmp, real):
    real_ = real #functions.read_image(opt)
    in_s = 0
    scale_num = 0
    ...
    while scale_num<opt.stop_scale+1:
        ...
        print('\n')
        startT = timer()
        print('scale_%d_start=' % (scale_num), startT)
        z_curr,in_s,G_curr = train_single_scale(D_curr,G_curr,reals,Gs,Zs,in_s,NoiseAmp,opt)
        endT = timer()
        print('scale_%d_end=' % (scale_num), endT)
        print('scale_%d_total_time = ' % (scale_num), endT-startT)
        print('\n')
